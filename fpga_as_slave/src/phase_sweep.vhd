        ...
        if rising_edge(CLK) then
            if RST = '1' then
                ...
                case state is
                    ...
                    -- BOOT emits one SWEEP_DONE pulse so sweep_engine can
                    -- start the first phase measurement sequence.
                    when BOOT =>
                        ...
                        retry_count    <= (others => '0');
                        done_r         <= '1';
                        incdec_r       <= '1'; -- Default to increment
                        -- Move to IDLE state, wait for first sweep request
                        state <= IDLE;
                        -- NOTE: This is a single-cycle pulse because done_r
                        -- defaults back to '0' each cycle above the case stmt.

                    when IDLE =>
                        incdec_r <= '1'; -- Maintain increment
                    ...
                    when READY =>
                        done_r   <= '1';
                        incdec_r <= '0'; -- Drop low momentarily to avoid Map optimizing to global VCC
                        state    <= IDLE;
